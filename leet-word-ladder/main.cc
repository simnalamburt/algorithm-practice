...
namespace {
  bool isDifferBySingleLetter(string_view A, string_view B) {
...
public:
  int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
    ...
    const auto& vertices = wordList;
    size_t V = wordList.size();
    ...
    vector<int> DIST(V, 2100000000);
    auto target_it = find(vertices.cbegin(), vertices.cend(), endWord);
    if (target_it == vertices.cend()) { return 0; }
    size_t target = distance(vertices.cbegin(), target_it);
    ...
    while (!Q.empty()) {
      ...
      for (size_t i = 0; i < V; ++i) {
        ...
        DIST[i] = new_dist;
        Q.push(make_pair(new_dist, i));
      }
    }

    if (DIST[target] == 2100000000) { return 0; }
    return DIST[target];
  }
};
...
int main() {
  ...
  printf("%d\n", Solution().ladderLength("nanny", "aloud", wordList));
}
